...
class pssm_rule_new(rule_object):
	...
	def create_prediction_data(self, discri_dict, **kwargs):
		...
		if 'sub_matrix' in list(kwargs.keys()):
			sub_matrix = pssm_func.get_sub_matrix(name = kwargs['sub_matrix'], gap_importance = gap_imp)
		else:
			sub_matrix = pssm_func.get_sub_matrix(name = 'basic', gap_importance = gap_imp)


		df = pssm_func.all_vs_all_pssm_cons(pssm_dict, sub_matrix= sub_matrix)	#get the cons_scores for all pssms
